...
module tb_eulsukdo_example_top ();
    ...
    // Task to fetch 2 instructions concurrently (2 Decode slots)
    task fetch_double_instruction;
        reg [31:0] target_pc;
        reg [31:0] word_addr;
        ...
        if (o_im_pc_valid) begin
            ...
            @(negedge clk);
            if (word_addr < 512) begin
                inst0  = instruction_memory[word_addr];
                valid0 = (inst0 == 32'h0010_0073 || inst0 === 32'hxxxxx) ? 1'b0 : 1'b1;
            end else begin
                inst0  = 32'h0;
                valid0 = 1'b0;
                end_pc_flag = 1'b1;
            ...
            if (word_addr + 1 < 512) begin
                inst1  = instruction_memory[word_addr + 1];
                valid1 = (inst1 == 32'h0010_0073 || inst1 === 32'hxxxxx || inst0 == 32'h0010_0073) ? 1'b0 : 1'b1;
            end else begin
                inst1  = 32'h0;
                valid1 = 1'b0;
            end

            i_im_inst       = {inst1, inst0};
            i_im_inst_valid = {valid1, valid0};

            if (inst0 == 32'h0010_0073 || inst1 == 32'h0010_0073) begin
                end_pc_flag = 1'b1;
            end
        end else begin
            i_im_inst_valid = 2'b00;
        end
    endtask

    // Virtual memory access handling for the Memory Core
    ...
    initial begin
        ...
        $display("=========================================");
        $display("🚀 Starting EULSUKDO Example Top Testbench");
        $display("   - Decode Width: 2 | Issue Width: 5");
        $display("=========================================");
