...
Register File의 주소로 **내부 레지스터 번호**를 사용하고, 
Register File의 데이터로 **내부 레지스터의 준비 여부**를 저장합니다.  
...
- jump: Immediate 값을 이용한 점프 명령 여부
- jump_reg: 레지스터 값을 이용한 점프 명령 여부
- branch: 분기 명령 여부
- new_pc: 점프/분기로 변경되거나 변경될 수 있는 PC. *단, jump_reg 발생에서는 사용하지 않음*
